...
const Index: React.FC = () => {
  ...
  const handleCreateTask = async () => {
    ...
    try {      // Use addTask based on the type definition
      await addTask({
        title: newTaskTitle,
  ...
  const xpProgress = Math.min(Math.round((currentXP / xpForNextLevel) * 100), 100);

  return (
    <div className="space-y-6 mx-auto p-4 container">      {/* Chào mừng người dùng */}
      <div className="flex justify-between items-center">
        <div>
          <h1 className="font-bold text-3xl">Welcome back, {user.username || user.first_name}!</h1>
      ...
      <div className="gap-4 grid grid-cols-1 md:grid-cols-4">
        ...
        <Card>
          <CardContent className="flex flex-col justify-center items-center p-4">
            ...
            <h3 className="font-bold text-2xl">{user.dailyLoginStreak || 0} days</h3>
          </CardContent>
        </Card>
      </div>
        ...
        <TabsContent value="overview">
          <Card>
            <CardHeader>
              ...
              <CardDescription>Your task completion and level progress</CardDescription>
            </CardHeader>
            <CardContent className="space-y-4">
              ...
              <div className="space-y-2">
                <div className="flex justify-between">
                  ...
                  <span className="font-bold text-sm">Level {user.level || 1} • {currentXP}/{xpForNextLevel} XP</span>
                </div>
        ...
        <TabsContent value="tasks">
          <Card>
            <CardHeader>
              ...
              <CardDescription>Manage and track your current tasks</CardDescription>
            </CardHeader>
            <CardContent>
              {isTasksLoading ? (
                ...
                <div className="divide-y">
                  {pendingTasks.slice(0, 5).map((task) => (
                    <div key={task._id} className="flex justify-between items-center py-3">
                      <div>
                        <h4 className="font-medium">{task.title}</h4>
                        <p className="text-muted-foreground text-sm">{task.description?.substring(0, 60) || 'No description'}</p>                        <div className="flex items-center gap-2 mt-1">
                          <Badge variant="outline" className="text-xs">XP: {task.xpReward || 0}</Badge>
                          {task.deadline && (() => {
                            ...
                            return (
                              <Badge variant="outline" className={badgeClasses}>
                                ...
                                {diffDays < 0 ? " (Overdue)" : diffDays === 0 ? " (Today!)" : diffDays === 1 ? " (Tomorrow!)" : ""}
                              </Badge>
                            );
                          })()}
                        </div>
                        ...
                        size="sm" 
                        variant="outline" 
                        onClick={() => task._id && handleConfirmComplete(task._id)}
                ...
                <p className="py-4 text-center">You don't have any pending tasks. Great job!</p>
              )}
            </CardContent>
          </Card>
        </TabsContent>
        ...
        <TabsContent value="rewards">
          <Card>
            <CardHeader>
              ...
              <CardDescription>Complete tasks to earn tokens and rewards</CardDescription>
            </CardHeader>
            <CardContent>
              <div className="flex justify-between items-center mb-4">
                <span>Current Tokens: <span className="font-bold">{user.tokens || 0}</span></span>
                <Button variant="outline" size="sm" onClick={() => navigate('/store')}>Visit Store</Button>
              </div>
              
              {/* Danh sách phần thưởng có sẵn - có thể thay thế bằng dữ liệu thực */}
              <div className="gap-4 grid grid-cols-1 md:grid-cols-2">
                <div className="flex justify-between items-center p-3 border rounded-lg">
                  <div>
                    <h4 className="font-medium">Premium Badge</h4>
                    <p className="text-muted-foreground text-sm">Show off your premium status</p>
                  </div>
      ...
      <Dialog open={isConfirmDialogOpen} onOpenChange={setIsConfirmDialogOpen}>
        <DialogContent>
          ...
          <p>Are you sure you want to mark this task as complete? This action cannot be undone.</p>
          <DialogFooter>
